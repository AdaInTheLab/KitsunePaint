...
// NOTE: the auto-cropper used to fire here on non-square uploads, but it
// was getting in the way of the multi-block grid feature ~ users with
// e.g. a 1024x256 strip want to slice it 4x1, not crop it square first.
// Now we let any dimensions through and the build pipeline handles it
// (single-block silently center-crops, multi-block slices then crops
// each tile to square ~ see buildModlet.ts).
...
export function TextureUploader({ onTextureSelect, onPBRSelect }: TextureUploaderProps) {
  ...
  // Simple mode handler ~ accept any image dimensions; the build pipeline
  // handles single-block (center-crop) vs multi-block (slice + crop tiles).
